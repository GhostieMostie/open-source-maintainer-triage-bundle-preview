...
It also includes Deposit Radar, a conservative active-lane scanner that checks PR/issue state, mergeability, labels, crowding, and maintainer-owned trigger words before anyone posts another nudge.
...
Use the free previews first if you only need one checklist. Buy the bundle when you want all three paid kits plus the start guide, buyer-routing guide, manifest, reward scanners, and one-day rollout workflow in one download.
